...
- **D1 美术风格定位**：写实 / 动画（二次元）/ 两者都要？→ 直接影响基模与一致性方法选型（A2），也决定
  [env-survey.md](env-survey.md) 该重点找哪些模型。**建议先定。**
- **D2 角色来源**：用户**用文本描述**生成角色 / **上传参考图**（真人或既有形象）/ 两者？→ 决定 identity
  方法（参考图 → InstantID / PuLID 很强；纯文本是另一套）与资产录入流程。**建议先定。**
- **D3 视频交付规格（MVP 默认）**：分辨率（720p / 1080p？）、宽高比（16:9 / 9:16？）、单镜头时长（3–5s？）、
  帧率。→ 影响管线 / 显存 / 成本。可先给默认，后续微调。
- **D4 导出工程格式**：FCPXML / Premiere XML / DaVinci / 片段+EDL / 仅片段文件夹？→ 定义 MVP 终点交付物
  （roadmap「导出工程 B 方案」）。
- **D5 MVP 音频范围**：确认 MVP **完全无音频**（对白 / 配音 / 音轨全归后置剪辑 Agent）？
- **D6 使用范围**：自用工具 / 计划给他人用 / 分发？→ 影响多用户、打磨程度、签名公证优先级。
