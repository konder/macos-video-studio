...
> **已更新**：项目层不再是节点画布，改为「**导演层 = 制片管理面（围绕分镜头）**」——见下方 §6 与
> [native-ui.md](native-ui.md) ②+③。本图的节点画布画法仅作历史参考，依赖关系改用结构化呈现（检查器/关系面板）。

![画布优先-项目层（已被 §6 导演层取代）](mockups/02-canvas-first-project.svg)
