...
- **5090 = 生成主力**（compute 快、模型已装、可直接下模型），承载 MVP 的图像+i2v 视频。
- **DGX 120G** 虽显存大，但**内部带宽受限、生成慢** → **不作为生成/交互路径**；维持其 **LLM 网关(LiteLLM)** 角色（延迟容忍）。高分辨率 LoRA 训练若需要，优先**租云 GPU**而非 DGX。
- **MVP 一致性主路径 = 5090 上的参考条件（i2v / reference-to-video）**，绕开训练瓶颈。
