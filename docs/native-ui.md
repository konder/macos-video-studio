...
>
> 进度:**核心问题 ①+④(Agent×画布协同)已对齐 v0.1**;其余为待讨论。
...
按「三层」框架排序,①+④ 已对齐(上),以下待逐个深入:

**第二层(能否做出来)——讨论中,已出示意图(见 [mockups.md](mockups.md) §6–8)**
- **双层画布导航**:架构「钻入」(只实例化一个镜头的图)+ 手感「缩放」(zoom-to-enter);面包屑钻取栈 /
  缩略地图 / 聚焦模式防迷路;项目层毛球用「选中即高亮、其余 ghost」治理。→ [图6](mockups/06-canvas-navigation.svg)
- **自研画布性能**:倾向**混合渲染**——Metal 基底(连线 + 全节点 LOD 代理)+ 可回收 SwiftUI 叠加池
  (仅视口内+近景+选中/编辑的少量节点物化);视口剔除 + quadtree 命中测试;高频反馈走 Metal 叠层。
  渲染架构是最大单点决策(全 SwiftUI / 全 Metal / 混合),**待最终拍板**。
  → [图7 渲染架构](mockups/07-render-architecture.svg) · [图8 LOD 分级](mockups/08-lod-tiers.svg)
