...
- **导演 / 制片 Agent**：拆剧本 → 镜头清单，排镜头，管一致性，调度本地 / 云，做高层规划。
  对应 UI 的**导演层**（制片管理面）。
- **搭图 Agent**：为每个具体生成任务造 / 改 ComfyUI 工作流（Graph IR）。对应 UI 的**技术层**（任务执行 flow）。

> Agent 与人是同一份 IR 的对称操作端（无特权写路径）；Agent 改图如何以 diff / 接管 / 撤销在画布上呈现，
> 见 [native-ui.md](native-ui.md) 的「Agent 与画布的协同编辑」。
