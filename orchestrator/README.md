...
│  ├─ agent.py      # Claude tool-use 手动循环(claude-opus-4-8 + adaptive thinking)
...
cp .env.example .env   # 填 ANTHROPIC_API_KEY，确认 COMFY_URL
...
- Agent = 最新 Claude 模型 + tool-use;四工具与 native-ui 的「op / 无特权写路径」一致。
